...
import { LinkedContactExternalWithDocuments } from '../contact_external';
...
export class CreateLinkedContactExternalForDocuments {
  ...
  save(){
    ...
    .then(data => {
      ...
      if(data.status){
        this.navCtrl.push(LinkedContactExternalWithDocuments , { document_id : this.params.get('document_id') } )
      }
  ...
  addContact(){
    this.navCtrl.push(ContactExternalCreateScreen1, { page: 'linked_documents', request_id : this.params.get('document_id') })
  }
