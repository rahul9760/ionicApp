...
import { LinkedDocumentsWithInfrastructure } from '../documents';
...
export class CreateLinkedDocumentForInfrastructure {
  ...
  save(){
    ...
    .then(data => {
      ...
      if( data.status ){
        this.navCtrl.push(LinkedDocumentsWithInfrastructure , { location_id : this.params.get('location_id') } )
      }
  ...
  addDocument(){
    this.navCtrl.push(Screen1, { request_id: this.params.get('location_id'), page: 'linked_infrastructure' } )
  }
