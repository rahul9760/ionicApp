...
import { LinkedContactExternalWithInfrastructure } from '../contact_external';
...
export class CreateLinkedContactExternalForInfrastructure {
  ...
  save(){
    ...
    .then(data => {
      ...
      if( data.status ){
        this.navCtrl.push(LinkedContactExternalWithInfrastructure , { location_id : this.params.get('location_id') } )
      }
  ...
  addContact(){
    this.navCtrl.push(ContactExternalCreateScreen1, { request_id: this.params.get('location_id'), page: 'linked_infrastructure' } )
  }
