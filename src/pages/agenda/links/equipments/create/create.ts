...
import { LinkedEquipmentWithAgenda } from '../equipments';
...
@Component({
  ...
  providers: [TasksServiceProvider,EquipmentServiceProvider,FunctionsHelper]
})

export class CreateLinkedEquipmentForAgenda {
  ...
  save(){
    ...
    .then(data => {
      ...
      if( data.status ){
        this.navCtrl.push(LinkedEquipmentWithAgenda , { task_id : this.params.get('task_id')} )
      }
